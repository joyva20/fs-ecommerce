...
// RUTE BARU: Dapatkan rekomendasi untuk sebuah produk
// GET /api/products/:id/recommendations
router.get('/:id/recommendations', async(req, res) => {
    try {
        const currentProductId = req.params.id;

        // 1. Dapatkan produk saat ini untuk mengetahui brand-nya
        const currentProduct = await Product.findById(currentProductId);

        if (!currentProduct || !currentProduct.brand) {
            // Jika produk tidak ditemukan atau tidak punya brand, kirim array kosong
            return res.json({ products: [] });
        }

        // 2. Logika Rekomendasi (Contoh: berdasarkan brand yang sama)
        // Cari produk lain dalam brand yang sama, kecualikan produk saat ini.
        const recommendations = await Product.find({
                brand: currentProduct.brand, // Cari berdasarkan brand yang sama
                _id: { $ne: currentProductId } // Gunakan $ne (not equal) untuk mengecualikan produk itu sendiri
            })
            .limit(4); // Batasi hanya 4 produk rekomendasi
        ...
        res.status(500).json({
            error: 'Permintaan Anda tidak dapat diproses. Silakan coba lagi.'
        });
    }
});
